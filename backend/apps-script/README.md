...
- `posts`: 게시글 ID, 작성자 ID, 카테고리, 제목, 요약, 태그, 본문, 생성일
...
버전 1.3.0은 게시글 생성·목록·상세·수정·삭제를 지원합니다. 수정과 삭제는 세션의 사용자 ID와 게시글 작성자 ID가 일치할 때만 허용됩니다.
기존 배포를 수정해 URL을 유지하고, 프론트엔드의 변경된 `js/script.js`도 사이트에 반영해야 합니다.
메인 카드, 글 목록, 상세 페이지는 `posts` 탭의 실제 데이터를 조회하며 프로필에서 본인이 쓴 글을 수정하거나 삭제할 수 있습니다.
`health`의 `setupComplete`는 초기화 실행 이력이며 현재 DB 접근이나 저장 성공을 보증하지 않습니다.
